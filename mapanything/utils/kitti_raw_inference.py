# usage: uv run scripts/kitti_raw_inference.py --model mapanything --dataset-root /home/kobayashi/dataset/kitti/raw_data/ --window-size 50
...
DEFAULT_DATASET_ROOT = Path("/home/kobayashi/dataset/kitti/raw_data")
DEFAULT_OUTPUT_ROOT = Path("/home/kobayashi/dataset/kitti/raw_inference_outputs")
DEFAULT_MAPANYTHING_MODEL_ID = "facebook/map-anything"
DEFAULT_WINDOW_SIZE = 100
SUPPORTED_MODELS = ("mapanything", "pi3x", "da3_nested")
SUPPORTED_CAMERAS = ("image_02", "image_03")
...
class KittiRawInferenceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        normalized_cameras = list(self.cameras) if self.cameras is not None else ["image_02"]
        ...
        if self.drives is not None:
            object.__setattr__(self, "drives", list(self.drives))
...
class ModelSpec:
    ...
    norm_type: str
    patch_size: int
    loader: Callable[[KittiRawInferenceConfig, torch.device], torch.nn.Module]
    adapter: Callable[
        [torch.nn.Module, list[dict[str, Any]], torch.device, KittiRawInferenceConfig],
...
def _discover_date_dirs(
    dataset_root: Path,
    selected_dates: list[str] | None,
) -> list[Path]:
    if not dataset_root.exists():
        raise FileNotFoundError(f"KITTI Raw dataset root not found: {dataset_root}")

    available = {
        date_dir.name: date_dir
        for date_dir in sorted(dataset_root.iterdir())
        if date_dir.is_dir()
    }
    if not available:
        raise ValueError(f"No date directories found under {dataset_root}")

    if not selected_dates:
        return [available[name] for name in sorted(available)]

    missing = [date for date in selected_dates if date not in available]
    if missing:
        raise ValueError(
            f"Requested date directories not found under {dataset_root}: {', '.join(missing)}"
        )

    return [available[date] for date in selected_dates]


def discover_raw_drive_dirs(
    dataset_root: Path,
    selected_dates: list[str] | None,
    selected_drives: list[str] | None,
) -> list[Path]:
    date_dirs = _discover_date_dirs(dataset_root, selected_dates)
    ...
    for date_dir in date_dirs:
        for drive_dir in sorted(date_dir.iterdir()):
            if not drive_dir.is_dir():
                continue
            if not drive_dir.name.endswith("_sync"):
                continue
            discovered.append(drive_dir)
            available[drive_dir.name] = drive_dir

    if not discovered:
        raise ValueError(
            f"No KITTI Raw drive directories ending with '_sync' were found under {dataset_root}."
        )

    if not selected_drives:
        return discovered

    missing = [drive for drive in selected_drives if drive not in available]
    if missing:
        raise ValueError(
            f"Requested drives not found under {dataset_root}: {', '.join(missing)}"
        )

    return [available[drive] for drive in selected_drives]


def parse_kitti_raw_intrinsics(date_dir: Path, camera: str) -> np.ndarray:
    if camera not in PROJECTION_KEY_BY_CAMERA:
        raise ValueError(f"Unsupported camera '{camera}'. Expected one of {SUPPORTED_CAMERAS}.")

    calib_path = date_dir / "calib_cam_to_cam.txt"
...
def load_camera_image_paths(
    drive_dir: Path,
    camera: str,
) -> tuple[Path, list[Path]]:
    camera_dir = drive_dir / camera / "data"
    if not camera_dir.is_dir():
        return camera_dir, []
    return camera_dir, sorted(camera_dir.glob("*.png"))
...
def save_batch_output(
    ...
    np.savez(
        ...
        date=np.array(date_name),
        drive=np.array(drive_name),
...
def run_drive_camera_job(
    ...
    camera_dir, image_paths = load_camera_image_paths(drive_dir, camera)
    if not camera_dir.is_dir():
        print(f"Warning: Missing camera directory: {camera_dir}, skipping.")
        return

    if not image_paths:
        print(f"Warning: No images found in {camera_dir}, skipping.")
        return

    date_name = drive_dir.parent.name
    intrinsics = parse_kitti_raw_intrinsics(drive_dir.parent, camera)
    save_dir = output_dir_for_drive(
    ...
    if config.resume and should_resume_skip_job(save_dir, len(image_paths), config.window_size):
        print(f"Skipping completed job: {drive_dir.name}/{camera}")
        return
...
def run_kitti_raw_inference(config: KittiRawInferenceConfig) -> None:
    device = resolve_device(config.device)
    model_spec = MODEL_SPECS[config.model]
    drive_dirs = discover_raw_drive_dirs(config.dataset_root, config.dates, config.drives)
    jobs = [(drive_dir, camera) for drive_dir in drive_dirs for camera in config.cameras]
    resolution_label = get_resolution_label(model_spec, config)

    if config.resume:
        pending_jobs: list[tuple[Path, str]] = []
        for drive_dir, camera in jobs:
            camera_dir, image_paths = load_camera_image_paths(drive_dir, camera)
            if not camera_dir.is_dir() or not image_paths:
                pending_jobs.append((drive_dir, camera))
                continue
        ...
        jobs = pending_jobs
        if not jobs:
            print("All requested KITTI Raw jobs are already processed; nothing to do.")
            return

    model = model_spec.loader(config, device)

    for drive_dir, camera in tqdm(jobs, desc="drive/camera jobs"):
        run_drive_camera_job(
            model=model,
            model_spec=model_spec,
            drive_dir=drive_dir,
            camera=camera,
